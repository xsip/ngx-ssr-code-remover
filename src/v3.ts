        ...
        if (rawCode.substring(definition.start, definition.end).includes('.RemoveOnServe')) {
            ...
            if (definition.type === 'ExpressionStatement') {
                ssrFns.push({
                    ...
                    classIndex: i - 1,
                    classInstance: program.body[i - 1] as acorn.VariableDeclaration,
...
function removeSsrFnBody(file: string, output: string, decoratedFns: SsrFnObj[], doneMethods: DoneMethods) {
